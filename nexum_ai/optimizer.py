...
class SemanticCache:
    ...
    def __init__(
        self, similarity_threshold: float = 0.95, cache_file: str = "semantic_cache.pkl"
    ) -> None:
    ...
    def put(self, query: str, result: str) -> None:
        ...
        query_vec = self.vectorize(query)
        self.cache.append({"query": query, "vector": query_vec, "result": result})
    ...
    def load_cache(self, filepath: Optional[str] = None) -> None:
        ...
        if os.path.exists(filepath) and filepath.endswith(".pkl"):
            try:
                ...
                print(
                    f"Semantic cache loaded from {filepath} ({len(self.cache)} entries)"
                )
                print(
                    "Note: Converting legacy pickle cache to JSON format for security"
                )
    ...
    def load_cache_json(self, filepath: Optional[str] = None) -> None:
        ...
        if os.path.exists(filepath):
            try:
                ...
                print(
                    f"Semantic cache loaded from JSON: {filepath} ({len(self.cache)} entries)"
                )
...
class QueryOptimizer:
    ...
    def explain_action(
        self, query: str, available_actions: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        return {
            "state": state,
            "q_values": q_values,
            "best_action": best_action[:20],
            "epsilon": self.epsilon,
            "would_explore": self.epsilon > 0,
            "explanation": (
                f"With ε={self.epsilon:.4f}, agent would explore "
                f"{self.epsilon * 100:.1f}% of the time"
            ),
        }
...
def test_vectorization() -> Dict[str, Any]:
    ...
    return {"query": test_query, "vector": vector[:10], "dimension": len(vector)}
...
def explain_query_plan(
    ...
    result["execution_strategy"] = {
        ...
        "recommendation": "Use cached result"
        if would_hit_cache
...
def format_explain_output(explain_result: Dict[str, Any]) -> str:
    ...
    lines.append(
        f"│ Type: {query_type:<15} Complexity: {p['complexity_estimate']}/10              │"
    )
    lines.append(
        f"│ WHERE: {str(p['has_where_clause']):<8} JOIN: {str(p['has_join']):<8} AGG: {str(p['has_aggregation']):<8}     │"
    )
    lines.append(
        "└───────────────────────────────────────────────────────────────────┘"
    )
    ...
    lines.append(
        f"│ Entries checked: {entries_checked:<5} Threshold: {c['similarity_threshold']:<6}            │"
    )
    lines.append(
        f"│ Best similarity: {c['best_similarity']:<6} Would hit: {str(c['would_hit_cache']):<6}              │"
    )
    if c["top_matches"]:
        lines.append(
            "│ Top matches:                                                      │"
    ...
    lines.append(
        f"│ State: {state_display:<30} Epsilon: {r.get('epsilon', 0):<6}        │"
    )
    ...
    lines.append(
        f"│ Strategy: {strategy_display:<20} Est. latency: {e['estimated_latency']:<10}   │"
    )
    lines.append(
        f"│ Will cache: {str(e['will_cache_result']):<8}                                          │"
    )
