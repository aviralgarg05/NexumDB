...
class CostBasedOptimizer(QueryOptimizer):
    """
    Inherits QueryOptimizer
    Can add additional methods if needed
    """

    def step(self, state: str, action: str, reward: float, next_state: str) -> None:
        """Perform one learning step using inherited Q-learning update"""
        self.update(state, action, reward, next_state)
...
class CostBasedOptimizer:
    ...
    def explain_action(
        ...
        if state in self.q_table:
            q_values = {
                a: round(v, 4)
                for a, v in self.q_table[state].items()
            }
        else:
