...
class QLearningAgent:
    ...
    def __init__(
        ...
        print(
            f"RL Agent initialized: lr={learning_rate}, gamma={discount_factor}, epsilon={epsilon}"
        )

    def _get_state_key(
        self, query_length: int, cache_hit: bool, complexity: int
    ) -> str:
        """
    ...
    def update(
        ...
        print(
            f"Updated Q({state}, {action}) = {new_q:.4f} (reward={reward:.4f}, latency={latency_ms:.2f}ms)"
        )
    ...
    def decay_epsilon(self) -> None:
        ...
        if self.epsilon > self.epsilon_min:
            ...
            print(
                f"Episode {self.episode_count}: epsilon decayed to {self.epsilon:.4f}"
            )
    ...
    def load_state(self, filepath: Optional[str] = None) -> None:
        ...
        if os.path.exists(filepath):
            try:
                ...
                print(
                    f"Agent state loaded from {filepath}: {len(self.q_table)} states, epsilon={self.epsilon:.4f}"
                )
            except Exception as e:
...
def test_rl_agent() -> None:
    ...
    for i, scenario in enumerate(test_scenarios):
        ...
        action = agent.get_action(
            scenario["query_len"], scenario["cache_hit"], scenario["complexity"]
        )
