...
class NLTranslator:
    ...
    def __init__(self, model_path: Optional[str] = None, n_ctx: int = 2048) -> None:
        ...
        if model_path and os.path.exists(model_path):
            if Llama is None:
                print(
                    "Warning: llama-cpp-python not installed. NL translation will use fallback."
                )
                self.model = None
            else:
                try:
                    ...
                    self.model = Llama(
                        model_path=model_path, n_ctx=n_ctx, n_threads=4, verbose=False
                    )
            ...
            print(
                "Warning: No model path provided or download failed. NL translation will use fallback."
            )
